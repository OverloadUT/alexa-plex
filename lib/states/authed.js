...
var setup = function() {
    // We have an auth token so we can simply use that
    common.pinAuth = new common.PlexPinAuth({
        token: common.user.authtoken
    });
...
var onDeckIntent = function(request, response) {
    common.plex.query('/library/onDeck').then(function(result) {
        ...
        for(i = 0; i < result._children.length && i < 6; i++) {
            if(result._children[i].type == 'episode') {
                shows.push(result._children[i].grandparentTitle);
            } else if (result._children[i].type == 'movie') {
                shows.push(result._children[i].title);
            }
        }
