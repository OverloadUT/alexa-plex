...
/**
 * Initialize the Plex API objects.
 * @param {boolean} [forceReInit=false] Force the PMS API object to be recreated with the latest server settings on the user
 */
var initPlexAPI = function(forceReInit) {
    // TODO the Plex API object should probably live here rather than common, come to think of it
    common.pinAuth = new common.PlexPinAuth({
        token: common.user.authtoken || null
    });

    if(!common.plexTV) {
        common.plexTV = new common.PlexAPI({
            hostname: 'plex.tv',
            port: 443,
            https: true,
            authenticator: common.pinAuth,
            options: common.plexOptions
        });
    }

    // Create the plex API object if we don't have one but only if a server is configured
    if((!common.plex || forceReInit) && common.user.server) {
        common.plex = new common.PlexAPI({
            hostname: common.user.hostname,
            port: common.user.port,
            https: common.user.https,
            authenticator: common.pinAuth,
            options: common.plexOptions
        });
    }
...
var getServers = function() {
    return common.plexTV.query('/api/resources?includeHttps=1').then(function(resources) {
...
var getPlayers = function() {
    return common.plex.query('/clients').then(function(resources) {
...
var getOnDeck = function() {
    return common.plex.query('/library/onDeck');
};
