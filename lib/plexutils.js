...
function proxyPerform(api, player, options) {
    console.time("proxyPerform: " + options);
    return api.perform({
        uri: options,
        extraHeaders: {'X-Plex-Target-Client-Identifier': player.machineIdentifier}
    }).then(function(response) {
        console.timeEnd("proxyPerform: " + options);
        return response;
    })
}
...
/**
 * Send a "play" command to a player
 * @param {module:App~App} app
...
var play = function(app) {
    ...
    var uri = ''+playerURI+'/playback/play' +
        '&type=video' +
        '&commandID=2' + // TODO this is supposed to be an incrementing number on each request
        '';

    return proxyPerform(app.plex.pms, app.user.player, uri);
};
